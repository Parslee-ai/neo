...
- **bulk below the sentinel**, so the target exceeds the reasoning prompt's
  per-file character cap. Realistic — real source files exceed 3000 characters
  constantly — and it is what proves a pinned file is delivered UNCUT past a
  cap that would otherwise have taken it. The sentinel sits at the top so the
  round trip does not depend on which end a cut would have kept.
- a **second oversized file the prompt does not name** (`bulk_rel`), which is
  what keeps the truncation-marker assertion (G3-inv) non-vacuous. It used to
  be the target: naming a path only BOOSTED it, so it competed for delivery
  like anything else and the renderer cut it. Now naming a path pins the file
  and a pin is never cut by the renderer, so the specimen for "every cut is
  marked" has to be a file that arrived on rank alone. Without this the
  G3-inv tests would pass on a bundle in which nothing was ever cut.
...
class FixtureRepo:
    ...
    duplicate_rel: str = ""
    #: Repo-relative path of an oversized file the prompt does NOT name. It is
    #: what keeps G3-inv non-vacuous now that naming a path pins the file: a
    #: pin is delivered whole and the prompt renderer does not cut it, so the
    #: target can no longer be the specimen for "every cut is marked". This
    #: file is selected on rank alone and is therefore still cut.
    bulk_rel: str = ""
...
''' + _pad(
    "# Per-format readers, kept in this module for locality.",
    "\n\ndef read_format_{i}(path: Path) -> list[bytes]:\n"
    "    \"\"\"Read a version-{i} record file.\"\"\"\n"
    "    return path.read_bytes().split(b\"|\")[{i} % 3:]\n",
)
...
''' + _pad(
    "// Per-format readers, kept in this file for locality.",
    "\npublic static class RecordReader{i}\n{{\n"
    "    /// <summary>Reads a version-{i} record file.</summary>\n"
    "    public static byte[] Read(byte[] raw) => raw;\n}}\n",
)
...
''' + _pad(
    "// Per-format readers, kept in this module for locality.",
    "\n/** Reads a version-{i} record file. */\n"
    "export function readFormat{i}(raw: string): string[] {{\n"
    "  return raw.split(\"|\").slice({i} % 3);\n}}\n",
)
...
_SPECS: dict[str, dict] = {
    ...
    "csharp": {
        ...
        "target": _CS_TARGET,
        "bulk_rel": "src/IChecksumService.cs",

...
- **File selection ranked files without reading them.** `score_candidate` took `(rel_path, size, prompt_tokens, git_recent, entry_points)` — no content parameter. Files were opened inside `gather_context` *after* selection, only to chunk what had already been chosen. With no content signal, relevance came from a filename and a byte count, and the dominant term was `score -= 0.01 * size_kb`, uncapped, against a realistic positive signal of +0.6 to +2.1 — so a file with one keyword hit was unrankable above 60 KB. Measured for *"fix the fact store supersession threshold"*: `src/neo/memory/store.py` scored **0.000 and ranked 200th of 284**, because it is 162 KB. Ground truth across the eval set runs 31–177 KB against a corpus median of 10 KB; central files are large *because* they are central.

  This had been diagnosed once already, and patched with a seven-name stem whitelist that rescued `engine.py` (−0.13) and left `store.py` (−1.62) — a **12× disparity between two files of near-identical size, decided by whether someone had thought of the name**. The literature says the sign was wrong, not the magnitude: BugLocator's rVSM (Zhou et al., ICSE 2012) ranks *larger* files *higher* for this task, and BM25's `b` handles the real concern with bounded, corpus-derived normalization. Selection is now BM25 over file content with code-aware tokenization — identifiers split on camelCase and separators, emitting both the whole identifier and its parts, so a prompt saying "user by id" reaches `getUserById` — max-normalized per query at a named `CONTENT_WEIGHT = 3.0`. The size penalty and the stem whitelist are deleted; filename overlap drops from 0.6 to 0.15 per hit and becomes a tie-breaker; `EXPLICIT_PATH_BOOST` is untouched. Test files are demoted on the keyword path as well as the semantic one, which matters more under content BM25 than before, because a test file contains its subject's identifiers plus test scaffolding and is often the better lexical match. (`context_gatherer.py`, `file_retrieval.py`, #193, #194)

  Measured before/after on three flagship repositories, same instrument on both arms, against the baselines in `docs/eval-baselines-2026-08.md`:

  | repo | MRR | recall@10 |
  |---|---|---|
  | neo | 0.136 → **0.714** | 0.212 → **0.751** |
  | aieweb | 0.180 → **0.759** | 0.244 → **0.774** |
  | m365dotnet (C#, largest) | 0.051 → **0.738** | 0.097 → **0.883** |

  The cost is real and is stated rather than buried: reading every candidate on every call takes the m365dotnet warm-call battery from 10.54 s to 36.41 s. Peak RSS is unchanged. Moving this index onto disk beside the semantic catalog is the next goal in `docs/unified-store-plan.md`.

- **`--dry-run` could make real, billed LLM calls.** The flag exited in `cli.main` *before the engine was constructed*, so it printed the gathered file list and nothing else — while the documentation said it "assembles the full context (file selection, fact retrieval, constraints, four-layer assembly) and prints what would be sent to the LM". Three of those four never ran. Worse, the multi-agent panel does not reason through `self.lm`: `_build_car_role_factory` calls `create_adapter("car", model=m)` per role, so with `car-server` reachable — this project's documented normal setup, since the observer autostarts off it — a novel prompt under `--dry-run` ran the full panel against real models, spent real money, and printed ordinary output. **Measured: 4 real adapters built (planner, coder, critic, judge); now 0.**

  The prompt is recorded, never rebuilt: `neo.dry_run.RecordingLM` is a real `LMAdapter` in the engine's own `self.lm` slot, so the output is the exact `messages` list Neo hands the adapter — a renderer walking the context dict would be a second implementation of the seven prompt builders, free to drift. `DryRunComplete` derives from `BaseException` so `_process_guarded`'s `except Exception` cannot report a dry run as a `FAILED` lifecycle event, and `_deliberate`'s own handler cannot swallow it into the fast path. A dry run also mutates nothing: `FactStore(read_only=True)` makes `save()` a no-op at the single write choke point, which a new caller cannot forget. Three further defects were found while moving the stop into the engine — `--stdin-json --dry-run` never stopped at all, `--json --dry-run` broke both of its documented invariants (zero documents on stdout, no terminal event, and every source line beginning with `{` becoming a counterfeit event), and `--no-scan` left `gathered` unbound. (`cli.py`, `dry_run.py`, `engine.py`, #187)

- **The Execution Envelope dropped its tail in silence.** `prompt_section()` carried bare slices — `constraints[:12]`, `success_criteria[:8]`, `attempts[-3:]` — reaching the model through seven prompt builders. Under a prompt that says "satisfy these constraints", twelve of thirteen read as thirteen; the model cannot ask about what it was not told exists. The #178 sweep missed this module for the reason that sweep wrote down about itself: it looked for slices inside the known prompt builders, and this is a method on a dataclass reached indirectly through `_retrieve_context`. Every cut is now marked, caps unchanged, with `shown_of(..., tail=True)` rendering `[showing last 3 of 20]` where the tail survived — which end was kept is part of the meaning. The proof-aware execution sections added in the same window (validation gates, validation evidence, hypotheses) are marked too. `retrieval_query()` keeps its bare slices deliberately: it is embedded and compared by cosine, never read, so a marker there would be tokens in the query vector. (`execution_context.py`, `text_budget.py`, #187)

- **Ruby had no edge queries at all, and PHP had only imports.** Ruby was the one language present in `QUERIES` and absent from `EDGE_QUERIES`: every ruby file in every index contributed nodes to the graph and never a relationship — no `require`, no superclass, no mixin. PHP's `class Impl extends BaseImpl implements Greeter` produced nothing where the java, c_sharp and typescript equivalents each produce edges. Both gaps were found while writing the per-query yield tests for `--dry-run` and pinned there as failing-when-fixed assertions.

  The fix needed a second pass for the reason the C# comment 150 lines above it already warned about. `(base_clause (name) @base)` requires `name` as a **direct** child; the real shape is `base_clause → qualified_name → name`, and Ruby's is `superclass → scope_resolution → constant`. The narrow version turned `class B extends \Foo\Bar implements \ArrayAccess, Countable` into exactly one edge, and `class ApplicationRecord < ActiveRecord::Base` — the single most common inheritance statement in the Ruby ecosystem — into none. Silent *partial* extraction is worse than a total miss, because the consumer sees a plausible answer with no signal that most of it is absent. Ruby imports are ordinary method calls, so the query matches any call taking a string and the `#match?` predicate does the separating; that predicate is load-bearing enough to have its own test, since a tree-sitter that stopped applying predicates would keep compiling and start emitting an import edge per string-argument call. (`index/language_parser.py`, #190, #204)

- **Chunk slots were apportioned one per file per round regardless of how much each file had to say.** Now allocated by symbol count, so a file with forty functions is not rationed to the same share as one with two. (`index/project_index.py`, #194)

### Added

- **A release gate that makes one real LLM round trip per language.** Three generated single-language fixture repos (C#, TypeScript, Python), each carrying a prompt-named target with a unique sentinel symbol, gitignored junk in the same language, an agent-worktree duplicate, and enough bulk to be cut by the prompt renderer. Two gates share them: the guard-invariant battery (`pytest -m invariants`) asserts G1-inv..G3-inv per language with no model call and runs on **every PR**, and the round trip (`pytest -m roundtrip`) requires the model to name the sentinel, closing the gap between "the gatherer selected the file" and "the model saw it" — which is where C# sat for eight and a half months. `publish.yml`'s `build` job depends on the round trip, so a red language blocks the wheel; it fails rather than skips when `ANTHROPIC_API_KEY` is absent, because an absent credential is a red gate, not a green one. A red round trip names the layer that broke, since "that language broke" is an expensive wrong turn when the real cause was the parser, the provider, the network or the credential. (`tests/test_selection_invariants.py`, `tests/test_release_roundtrip.py`, `docs/release-gate.md`, #202)

- **A retrieval evaluation harness, and published baselines for it.** `tools/rank_eval.py` scores twelve hand-labelled prompts by recall@k on this repository; `tools/rank_mine_eval.py` mines cases from any repository's git history — commit subject as the query, the commit's changed non-test files as ground truth — and reports R@k and MRR through the real CLI rather than an in-process replica, because `gather_context` re-ranks after `score_candidate` and an in-process replica overstated R@10 by 0.14. `docs/eval-baselines-2026-08.md` publishes M1, M2 and G1-invariant baselines for neo, aieweb and m365dotnet with the exact commands, the machine, and every number that could not be produced together with its blocking reason. (`tools/`, `docs/`, #192, #201)

- **Proof-aware execution.** Validation gates, caller-supplied validation observations, and falsifiable hypothesis records on the execution envelope, so completion can be argued from evidence a caller supplied rather than from model self-confidence. (`execution_context.py`, `memory/execution_evaluation.py`)

### Changed

- `.gitignore` is implemented rather than approximated. The gatherer honours the repository's own ignore rules through the shared loader instead of a hand-maintained directory list that drifted from it. (`context_gatherer.py`, #186)
- The unified-store plan is written down: `docs/unified-store-plan.md` records the metrics, the guard invariants, the goal sequence and the review-and-merge governance for collapsing Neo's two file-selection lanes into one walker, one index and one retrieval front door.

### Documentation

- `docs/eval-baselines-2026-08.md` pins the M1 instrument by revision rather than by path, because #194 gives `tools/rank_eval.py` back to the labelled harness and moves the git-mined MRR harness to `tools/rank_mine_eval.py`. Quoting one under the other's name is the "same label, different instrument" error that harness documents in its own header.
- `docs/release-gate.md` is the doc of record for the two-half release gate: what each asserts, why one is free on every PR and the other is paid on release, how to run both locally, and how to dry-run the gate without cutting a release.
...
- **A crashed run recorded its exception type and nothing else.** Two genuine failures sat in a live ledger for eleven days as a bare `ValueError` with no way to learn what raised them. `_error_evidence` now captures the message and traceback alongside the type, independently — `format_exception` is already hardened against a hostile `__str__` and renders `<exception str() failed>`, so sharing one try block would discard a good traceback to avoid an error it had already survived. Both are redacted *before* truncation, because the other order can split a secret across the cut and leave the tail half unscrubbed. The traceback is elided from the middle and says so: `raise X from Y` renders the original cause first and the outer exception last, and the outer one is already stored twice over in `error_type` and `error_message`, so a tail-only cut spends the whole budget restating what is already recorded. (`engine.py`, `memory/store.py`, #175)

- **Curated hosts could not turn off implicit repository context.** `NeoInput` gains `allow_implicit_context`, and the CLI gains `--no-memory` alongside a `--no-scan` that now disables project-instruction discovery (`AGENTS.md`, `CLAUDE.md`, tool rule files) rather than only the directory scan. The default preserves standalone and existing host behaviour. The Codex plugin's six skills carry the matching boundary wording, since Codex calls Neo mid-loop with no visible agent boundary and needs stronger attribution than the Claude Code path. (`models.py`, `cli.py`, `engine.py`, `plugins/neo/`)
...
- **The release commands described a process that cannot work.** `prepare-release` told the operator to hand-edit the version in three files and named only three of the four that restate it, omitting the Codex plugin manifest — which is exactly the drift that let the package and the two manifests reach 0.41.0 / 0.37.0 / 0.19.0. It now says to edit `pyproject.toml` alone, run `make sync-version`, and never hand-edit the derived files. Its final step instructed `git push origin main --tags`, which fails outright against a protected branch. `ship-release` carried the same stale three-file list twice and said only that main "requires PR for merges", omitting that it requires an approving review and that GitHub refuses self-approval — the thing that actually stops a solo release. Both now record the `--admin` escape and that a rejected `gh pr merge` is the protection working. (#173)
- **`ship-release --continue` could tag an unmerged main.** Phase 5 pulled main and tagged it with no check that the release PR had merged, while Phase 6 published regardless — PyPI would carry the new version against a tag pointing at a commit still declaring the old one, with nothing downstream catching it. Phase 5 now verifies MERGED and greps the version out of `pyproject.toml` before tagging. Phase 6's claim that attaching `dist/` "triggers the workflow which publishes to PyPI" was also wrong in a way that hides well: the `build` job checks out the tag and builds its own artifact, so the local build is verification and the attachments are convenience copies — the two are normally byte-identical, which is why the distinction goes unnoticed. (#174)
